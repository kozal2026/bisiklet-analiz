[15:23, 01.04.2026] Erdal: import streamlit as st
import time
import random
...
st.markdown("""
    ...
    /* 2x2 DÜZENİ VE SABİT BUTON BOYUTU */
    div[data-testid="stHorizontalBlock"] {
        display: flex !important; flex-direction: row !important;
        flex-wrap: wrap !important; justify-content: center !important; gap: 15px !important;
    }
    ...
    .stButton>button {
        width: 100% !important;
        max-width: 250px !important; /* Buton genişliğini sabitledik */
        height: 60px !important;      /* Buton yüksekliğini sabitledik */
...
if not st.session_state.elendi and st.session_state.index < 12:
    ...
    st.error(f"Yanlış Cevap! Elendiniz. Kazancınız: {oduller[st.session_state.index-1] if st.session_state.index > 0 else '0 TL'}")
    if st.button("Yeniden Başla"):
